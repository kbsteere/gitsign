...
| enableSigstoreGo   |   true                           | If true (the default), use the sigstore-go libraries for signing and verification via the CMS↔bundle compatibility layer, for both online and offline Rekor modes. Set to `false` to fall back to the legacy CMS + Rekor path. The on-disk signature format is unchanged. See [docs/bundle-cms.md](./docs/bundle-cms.md) for more details. |
| rekorVersion       |   1                              | (Experimental) Rekor API version to upload signatures to. One of [1, 2] (default: 1).<br>1 - the legacy Rekor v1 REST API.<br>2 - the Rekor v2 / [rekor-tiles](https://github.com/sigstore/rekor-tiles) API. Can only be set when the sigstore-go signing path is enabled (`enableSigstoreGo=true`, the default). |
...
| offlineAccess      |   false                          | If true, request the `offline_access` OIDC scope during interactive authentication so the [credential cache](cmd/gitsign-credential-cache/README.md) daemon can keep a refresh token in memory and mint new signing certificates without opening the browser again. Only takes effect when `GITSIGN_CREDENTIAL_CACHE` is used. See the credential cache README for security considerations. |
| offlineAccessMaxAge |  24h                            | Maximum duration (Go [duration syntax](https://pkg.go.dev/time#ParseDuration)) the credential cache daemon may mint certificates from a refresh token before requiring a new interactive login. `0` disables the bound. |
...
| GITSIGN_ENABLE_SIGSTORE_GO   | ❌                 | true                             | If true (the default), use the sigstore-go libraries for signing and verification via the CMS↔bundle compatibility layer, for both online and offline Rekor modes. Set to `false` to fall back to the legacy CMS + Rekor path. The on-disk signature format is unchanged. See [docs/bundle-cms.md](./docs/bundle-cms.md) for more details. |
| GITSIGN_REKOR_VERSION        | ✅                 | 1                                | (Experimental) Rekor API version to upload signatures to. One of [1, 2] (default: 1).<br>1 - the legacy Rekor v1 REST API.<br>2 - the Rekor v2 / [rekor-tiles](https://github.com/sigstore/rekor-tiles) API. Can only be set when the sigstore-go signing path is enabled (`GITSIGN_ENABLE_SIGSTORE_GO=true`, the default). |
...
| GITSIGN_OFFLINE_ACCESS       | ✅                 | false                            | If true, request the `offline_access` OIDC scope during interactive authentication so the [credential cache](cmd/gitsign-credential-cache/README.md) daemon can keep a refresh token in memory and mint new signing certificates without opening the browser again. Only takes effect when `GITSIGN_CREDENTIAL_CACHE` is used. |
| GITSIGN_OFFLINE_ACCESS_MAX_AGE | ✅               | 24h                              | Maximum duration (Go [duration syntax](https://pkg.go.dev/time#ParseDuration)) the credential cache daemon may mint certificates from a refresh token before requiring a new interactive login. `0` disables the bound. |
